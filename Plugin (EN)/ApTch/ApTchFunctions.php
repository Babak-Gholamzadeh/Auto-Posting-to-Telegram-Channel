...
function addSettingLink($links) {
  $settingLink = '<a href="admin.php?page=telegram_settings">Settings</a>';
  ...
  if ($column == 'telegramColumn') {
    echo '<center class="telegramColumnCenter">
            <div class="telegramColumn">
              <span class="telegramLC" title="The number of Link Click via telegram"><span class="counterLC">'.( get_post_meta( $post_id, 'telegramLC' ) ? get_post_meta( $post_id, 'telegramLC')[0] : 0 ).'</span><sup>LC</sup></span>
...
function addMetaBox() {
  add_meta_box( "telegramMetaBox", "<i class='fas fa-paper-plane' style='color: #08c;'></i> Telegram Post Options", "metaBoxOptions", "post", "side", "high" );
}
  ...
  <div class='telegramMetaBox'>
    ...
    <div class='sending'>
      ...
      <span class='enableCaption'>
        <?php
        echo get_post_meta( $post->ID, 'telegramPostID') ? 'Update This Post' : 'Send This Post';
        ?>
      </span>
    </div>
    ...
    <div class='pinMSG'>
      ...
      <span class='enableCaption'>Pin This Post</span>
    </div>
